...
begin
  ...
  if (aImage=nil) or (aImage.isEmpty) then begin
    Result := false;
    fLog(LogInterfaceError,{$I %CURRENTROUTINE%},'FATAL: Image is nil');
    Exit;
  end;
  if not (aImage is TRGBAlphaImage) then begin
    Result := false;
    fLog(LogInterfaceError,aImage.className+'>'+{$I %CURRENTROUTINE%},'FATAL: Image type '+aImage.ClassName+' is not supported for operation.');
    Exit;
  end;
end;
