...
const
  DefaultAnimationDuration = 0.3; {in seconds}

const
  UninitializedIntCoord = -999;
...
{ Type of this Anchor }
type
  TAnchorSide = (asLeft, asRight, asTop, asBottom);
{ Which part of Parent does this Anchor align to? }
type
  TAnchorAlign = (noAlign, haLeft, haRight, haCenter, vaTop, vaBottom, vaMiddle);
{ Which part of the image is fitted to maintain proportions }
type
  TProportionalScale = (psNone, psWidth, psHeight);
...
type
  TAnchorList = specialize TObjectList<DAnchoredObject>;
...
type
  ...
  public
    ...
    procedure AnchorSide(const aSide: TAnchorSide; const aParent: DFloatContainer;
      const aAlign: TAnchorAlign; const Gap: integer = 0);
    procedure AnchorTop(const aParent: DFloatContainer; const aAlign: TAnchorAlign;
      const Gap: integer = 0);
    procedure AnchorBottom(const aParent: DFloatContainer;
      const aAlign: TAnchorAlign; const Gap: integer = 0);
    procedure AnchorLeft(const aParent: DFloatContainer;
      const aAlign: TAnchorAlign; const Gap: integer = 0);
    procedure AnchorRight(const aParent: DFloatContainer;
      const aAlign: TAnchorAlign; const Gap: integer = 0);
...
type
  TXYProcedure = procedure(const Sender: DAbstractElement; const x, y: integer) of object;
...
begin
  ...
  begin
    Log(LogInterfaceScaleError, _CurrentRoutine,
      'WARNING: NotifyAnchor should be copied, do it!');
  end;
...
begin
  ...
  if Dest.Owner <> Self.Owner then
  begin
    Log(LogInterfaceScaleError,
      _CurrentRoutine, 'WARNING: NotifyAnchor should be copied, do it!');
  end;
...
begin
  ...
  begin
    if (Anchor[asLeft].Anchor = nil) or (Anchor[asTop].Anchor = nil) or
      (Anchor[asRight].Anchor = nil) or (Anchor[asBottom].Anchor = nil) then
    begin
    ...
    begin
      case Anchor[asLeft].AlignTo of
        ...
        haCenter: cx1 := (Anchor[asLeft].Anchor.x1 + Anchor[asLeft].Anchor.x2) div 2;
          ...
          Log(LogInterfaceError, Owner.ClassName + '>' + _CurrentRoutine, 'Invalid Anchor align!')
      end;
      ...
      case Anchor[asBottom].AlignTo of
        {Pay attention, this is INVERT due to OpenGL}
...
begin
  ...
  begin
    ...
    begin
      ...
      AdjustToRealSize; {sets x2,y2,w,h}
    fInitialized := True;
  end
  else
    fInitialized := False;
...
procedure DFloatContainer.AnchorTo(const aParent: DFloatContainer;
  const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorChild(const aChild: DFloatContainer;
  const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorSide(const aSide: TAnchorSide;
  const aParent: DFloatContainer; const aAlign: TAnchorAlign; const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorTop(const aParent: DFloatContainer;
  const aAlign: TAnchorAlign; const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorBottom(const aParent: DFloatContainer;
  const aAlign: TAnchorAlign; const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorLeft(const aParent: DFloatContainer;
  const aAlign: TAnchorAlign; const Gap: integer = 0);
begin
  {StartProfiler}
...
procedure DFloatContainer.AnchorRight(const aParent: DFloatContainer;
  const aAlign: TAnchorAlign; const Gap: integer = 0);
begin
  {StartProfiler}
...
begin
  ...
  begin
    ...
    if (x1 = UninitializedIntCoord) or (y1 = UninitializedIntCoord) or
      (x2 = UninitializedIntCoord) or (y2 = UninitializedIntCoord) then
      fInitialized := False;
  end;
  Result := fInitialized;
...
begin
  ...
  begin
    ...
    case Animate of
      ...
      asFlyInTop, asFlyOutTop, asFlyInBottom, asFlyOutBottom, asFlyInLeft,
      asFlyOutLeft, asFlyInRight, asFlyOutRight:
      begin
        ...
        if (Animate = asFlyInLeft) or (Animate = asFlyInRight) or
          (Animate = asFlyInTop) or (Animate = asFlyInBottom) then
        begin
                         {$HINT Test needed}
          Last.x1 := Round(mx * Window.Width);
          Last.y1 := Round(my * Window.Width);
...
begin
  ...
  begin
    ...
    if {(Last.isInitialized) and} (Next.isInitialized) and
      ((DecoNow - AnimationStart < AnimationDuration)) then
    begin
  ...
  begin
    ...
    Log(LogInterfaceInfo, _CurrentRoutine,
      'Base is uninitialized, falling back to Current=Base');
  end;
...
begin
  ...
  if (xx >= Base.x1) and (xx <= Base.x2) and (yy >= Base.y1) and
    (yy <= Base.y2) then
    Result := True
  else
    Result := False;
...
begin
  ...
  Children.Add(Child);
  if (Child is DSingleInterfaceElement) then
    DSingleInterfaceElement(Child).Parent := Self;
...
begin
  ...
  if (tmp <> nil) and (tmp is DSingleInterfaceElement) and
    (DSingleInterfaceElement(tmp).CanMouseOver){ and (tmp.base.opacity>0)} then
